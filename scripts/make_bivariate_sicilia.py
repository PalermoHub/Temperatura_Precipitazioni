"""Genera i dati per una mappa bivariata (X × Y), comuni Sicilia.

Legge data/processed/comuni_sicilia_clima.gpkg e produce, per la coppia di
variabili scelta:
...
SRC = "data/processed/comuni_sicilia_clima.gpkg"
...
def build_bivariate(var_x, var_y, label_x, label_y, unit_x, unit_y, map_id, suffix, pal, periodo, fonte):
    ...
    config = {
        "map_id":      map_id,
        "title":       f"{label_x} × {label_y} · Comuni di Sicilia",
        "label_x":     label_x,
        "label_y":     label_y,
        "unit_x":      unit_x,
        "unit_y":      unit_y,
...
if __name__ == "__main__":
    FONTE = "climatologylab.org (TerraClimate) + confini gbvitrano.it/anncus"
